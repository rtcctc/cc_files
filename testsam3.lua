...
local function drawVoteWindow(topic,yes,no,abstain)
    ...
    local greenW =
        math.floor((yes / total) * width)

    local redW =
        math.floor((no / total) * width)

    local yellowW =
        math.floor((abstain / total) * width)

    local used = greenW + redW + yellowW

    local grayW = width - used
    ...
    for i=1,greenW do
        writeAt(x,mh-4," ",nil,colors.green)
        x=x+1
    end

    for i=1,redW do
        writeAt(x,mh-4," ",nil,colors.red)
        x=x+1
    end

    for i=1,yellowW do
        writeAt(x,mh-4," ",nil,colors.yellow)
        x=x+1
    end

    for i=1,grayW do
        writeAt(x,mh-4," ",nil,colors.gray)
        x=x+1
    end
...
local function runVote()
    ...
    local resultText = "REJECTED"
    local resultColor = colors.red
